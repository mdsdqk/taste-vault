...
<p>
  ...
  <a href="#the-vault-on-disk">The Vault on disk</a> &nbsp;·&nbsp;
...
TasteVault sits between the interfaces you notice and an AI coding agent.

You save the screens, components, and interactions you've decided are good, and
the ones you've decided are bad, from anywhere a design lives: a website, a
mobile app, a smartwatch face, a device's on-screen menu. Each one keeps its
evidence and a note in your own words on why.

You browse the collection through the **Portal**. A retrieval layer will later
feed your agent the few relevant examples, and the relevant guard rails, before
it designs anything.

Four commitments set the shape of the project:

- **Your reasoning is the point.** Every Reference carries a *User Note*: plain
  prose on *why* you saved it. It stays the ground truth. Anything an AI derives
  later is stored separately and marked as derived.
- **Taste has two poles.** What you want to *avoid* is saved exactly like what
  you want to emulate, with one optional `sentiment: negative` field, and
  carried through every phase as a guard rail for the agent.
- **Retrieval, not generation.** TasteVault never produces an interface. It
  feeds a small relevant set to whoever is designing, you or your agent, and
...
A **Reference** is one folder inside it:
...
the file dates. Metadata is enrichment, never a contract. The stance is recorded
in [`docs/adr/0001-references-degrade-gracefully.md`](docs/adr/0001-references-degrade-gracefully.md).
...
`npx create-taste-vault` and `npx create-taste-vault init` do the same thing.
This clones the latest release into `./taste-vault` and installs dependencies.
Then:
...
To clone by hand you'll need pnpm 11 (`corepack enable` on Node 22).
...
| <kbd>pnpm new-ref</kbd> | Scaffold a well-formed Reference folder. Every prompt is optional. |
...
The authoring guide covers the folder shape, every frontmatter field, and how to
save a negative Reference: [`references/readme.md`](references/readme.md).
...
**`apps/server`** is Fastify + TypeScript. It scans and watches `references/`,
parses frontmatter, renders each User Note to sanitised HTML, and serves a JSON
API plus the raw asset files. Edits made in the Portal are written back to disk
in the same on-disk format `pnpm new-ref` produces. A Server-Sent Events stream
...
**`apps/web`** is Vite + React + TypeScript, Tailwind + Radix, self-hosted fonts.
It has two views. The **Library** is a gallery wall of screenshot plates hung by
rating, with a filter/sort band and tag facets. **Reference detail** sets the
User Note large next to the identity fields. Client-side fuzzy search runs over
...
The shared vocabulary (Reference, Sentiment, Vault, Portal, Evidence, User Note)
is fixed in [`CONTEXT.md`](CONTEXT.md). The full product spec is
...
Phase 1, the manual library, is what's built today. The
...
- **Phase 3: Focused and interactive capture.** Save one element in isolation;
  record `before → action → after` for hovers, menus, and modals; then
  animations, transitions, and multi-step flows.
- **Phase 4: AI analysis.** Derive observations, vocabulary, and a short "why
  this matters" note per Reference, always separate from the evidence, always
  marked as derived. Negative References yield anti-patterns from the same
  machinery.
- **Phase 5: Retrieval and MCP.** Lexical, semantic, and preference-aware
  retrieval behind an MCP server, so Claude Code can ask for the references
  relevant to what it's designing and get back a small, high-quality set with
  the guard rails included.
- **Phase 6: Agent augmentation and self-hosting.** Retrieval wired into the
